...
static void ReadArrayFunctionWithCoords(ClientContext &context,
  ...
  for (int d = 0; d < (int)data.dim_len; d++) {
    coords.push_back((uint64_t *)bf_util_pagebuf_get_coords(gstate.page, d));
  }

  if (data.format == TILESTORE_SPARSE_COO) {
    // TODO: Multi-tile array is not tested
    read = CooReader::PutData(data_p.bind_data, gstate, pagevals, coords, size,
                              output);
  } else if (data.format == TILESTORE_SPARSE_CSR) {
    read = CsrReader::PutData(data_p.bind_data, gstate, pagevals, coords, size,
                              output);
  } else {
    read =
        DenseReader::PutData(data_p.bind_data, gstate, pagevals, size, output);
  }
...
static void ReadArrayFunctionAll(ClientContext &context,
  ...
  while (!gstate.isFinished) {
    ...
    if (data.format == TILESTORE_DENSE) {
      read = DenseReader::PutData(data_p.bind_data, gstate, pagevals, size,
                                  output);
    } else {
      vector<uint64_t *> coords;
      for (int d = 0; d < (int)data.dim_len; d++) {
  ...
  for (auto it = input.named_parameters.begin();
    ...
    if (it->first == "coords") {
      ...
      for (int d = 0; d < (int)bind_data->dim_len; d++) {
        bind_data->requestedCoords.push_back(coords[d].GetValue<int64_t>());
      }
    }
  }
...
TableFunction ArrayExtension::GetReadArrayFunction() {
  TableFunction function = TableFunction(
      "read_array", {LogicalType::VARCHAR}, ReadArrayFunction, ReadArrayBind,
      ReadArrayGlobalStateInit, ReadArrayLocalStateInit);

  function.named_parameters["coords"] = LogicalType::LIST(LogicalType::INTEGER);
