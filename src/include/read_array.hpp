...
// calculate offsets for each attribute in the array pagebuf
...
// store uint64_t coordinate values to uint32_t vector.
// PreVision uses uint64_t as a type for coordinates, while this extension
// produces uint32_t. PreVision should use uint32_t instread of uint64_t.
...
//  update DuckDB vector for coordinate to global one (from local one)
...
struct ArrayReadGlobalState : public GlobalTableFunctionState {
 public:
  ...
  bool findNextTile() {
    ...
    // std::cout << "coords=" << currentCoordsInTile[0] << "," <<
    // currentCoordsInTile[1] << "," << currentCoordsInTile[2] << std::endl;
    return !isFinished;
  }
  ...
  void pin() {
    ...
    array_key key = {_arrname_char, dcoords.get(), dimLen, BF_EMPTYTILE_NONE};
  ...
  void unpin() {
    ...
    if (res != BFE_OK) {
      throw InternalException("Failed to unpin buffer: " + std::to_string(res));
    }
    page = NULL;
  }
