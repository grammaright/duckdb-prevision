...
vector<uint32_t> DenseReader::CalculateOffsets(
    const vector<int32_t> &tileCoords, const vector<uint64_t> &tileSize) {
  vector<uint32_t> offsets;
  for (uint32_t i = 0; i < tileSize.size(); i++) {
    offsets.push_back(tileCoords[i] * tileSize[i]);
  }
  return offsets;
}

uint64_t DenseReader::_Put3DNullableData(
  ...
  auto offsets = CalculateOffsets(gstate.currentCoordsInTile, data.tile_size);
  ...
  for (uint32_t i = 0; i < gstate.projection_ids.size(); i++) {
    ...
    if (dest == 0) {
      ...
      while (idx < max_processable && buf_idx < size) {
        ...
        uint32_t coord = buf_idx++ / (data.tile_size[1] * data.tile_size[2]);
      ...
      while (idx < max_processable && buf_idx < size) {
        ...
        uint32_t coord = (buf_idx++ / data.tile_size[2]) % data.tile_size[1];
        vec[idx++] = offsets[dest] + coord;
      }
...
uint64_t DenseReader::_Put3DNullableDataNoPrune(
    optional_ptr<const FunctionData> bind_data, ArrayReadGlobalState &gstate,
    char *pagevals, uint64_t size, DataChunk &output) {
  auto &data = bind_data->Cast<ArrayReadData>();
...
uint64_t DenseReader::_Put3DNullableDataNoPruneAndProjection(
    optional_ptr<const FunctionData> bind_data, ArrayReadGlobalState &gstate,
    char *pagevals, uint64_t size, DataChunk &output) {
  auto &data = bind_data->Cast<ArrayReadData>();
...
uint64_t DenseReader::_Put3DData(optional_ptr<const FunctionData> bind_data,
  ...
  for (uint32_t i = 0; i < gstate.projection_ids.size(); i++) {
    ...
    if (dest == 0) {
      auto vec = FlatVector::GetData<uint32_t>(output.data[i]);
      uint32_t offset = gstate.currentCoordsInTile[dest] * data.tile_size[dest];
...
uint64_t DenseReader::_Put3DDataNoPrune(
    optional_ptr<const FunctionData> bind_data, ArrayReadGlobalState &gstate,
    char *pagevals, uint64_t size, DataChunk &output) {
  auto &data = bind_data->Cast<ArrayReadData>();
...
uint64_t DenseReader::_Put3DDataNoPruneAndProjection(
    optional_ptr<const FunctionData> bind_data, ArrayReadGlobalState &gstate,
    char *pagevals, uint64_t size, DataChunk &output) {
  auto &data = bind_data->Cast<ArrayReadData>();
...
uint64_t DenseReader::_Put2DData(optional_ptr<const FunctionData> bind_data,
                                 ArrayReadGlobalState &gstate, char *pagevals,
                                 uint64_t size, DataChunk &output) {
  auto &data = bind_data->Cast<ArrayReadData>();
...
uint64_t DenseReader::_Put2DDataNoPrune(
    optional_ptr<const FunctionData> bind_data, ArrayReadGlobalState &gstate,
    char *pagevals, uint64_t size, DataChunk &output) {
  auto &data = bind_data->Cast<ArrayReadData>();
...
uint64_t DenseReader::_Put2DDataNoPruneAndProjection(
    optional_ptr<const FunctionData> bind_data, ArrayReadGlobalState &gstate,
    char *pagevals, uint64_t size, DataChunk &output) {
  auto &data = bind_data->Cast<ArrayReadData>();
...
uint64_t DenseReader::_Put2DNullableData(
    optional_ptr<const FunctionData> bind_data, ArrayReadGlobalState &gstate,
    char *pagevals, uint64_t size, DataChunk &output) {
  auto &data = bind_data->Cast<ArrayReadData>();
  uint64_t current_filled = output.size();
...
uint64_t DenseReader::_Put2DNullableDataNoPrune(
    optional_ptr<const FunctionData> bind_data, ArrayReadGlobalState &gstate,
    char *pagevals, uint64_t size, DataChunk &output) {
  auto &data = bind_data->Cast<ArrayReadData>();
...
uint64_t DenseReader::_Put2DNullableDataNoPruneAndProjection(
    optional_ptr<const FunctionData> bind_data, ArrayReadGlobalState &gstate,
    char *pagevals, uint64_t size, DataChunk &output) {
  auto &data = bind_data->Cast<ArrayReadData>();
...
uint64_t DenseReader::_Put1DData(optional_ptr<const FunctionData> bind_data,
                                 ArrayReadGlobalState &gstate, char *pagevals,
                                 uint64_t size, DataChunk &output) {
  auto &data = bind_data->Cast<ArrayReadData>();
...
uint64_t DenseReader::_Put1DDataNoPrune(
    optional_ptr<const FunctionData> bind_data, ArrayReadGlobalState &gstate,
    char *pagevals, uint64_t size, DataChunk &output) {
  auto &data = bind_data->Cast<ArrayReadData>();
  uint64_t current_filled = output.size();
...
uint64_t DenseReader::_Put1DDataNoPruneAndProjection(
    optional_ptr<const FunctionData> bind_data, ArrayReadGlobalState &gstate,
    char *pagevals, uint64_t size, DataChunk &output) {
  auto &data = bind_data->Cast<ArrayReadData>();
  uint32_t offset = gstate.currentCoordsInTile[0] * data.tile_size[0];
...
uint64_t DenseReader::_Put4DData(optional_ptr<const FunctionData> bind_data,
                                 ArrayReadGlobalState &gstate, char *pagevals,
                                 uint64_t size, DataChunk &output) {
  auto &data = bind_data->Cast<ArrayReadData>();
  uint64_t current_filled = output.size();
  ...
  for (uint32_t i = 0; i < gstate.projection_ids.size(); i++) {
    ...
    if (dest == 0) {
      ...
      for (uint64_t idx = 0; idx < local_remains; idx++) {
        uint32_t vec_idx = current_filled + idx;
        uint32_t buf_idx = gstate.cell_idx + idx;
        uint32_t coord = (buf_idx / (data.tile_size[2] * data.tile_size[3])) %
                         data.tile_size[1];
        vec[vec_idx] = offset + coord;
      }
    } else if (dest == 2) {
      ...
      for (uint64_t idx = 0; idx < local_remains; idx++) {
        uint32_t vec_idx = current_filled + idx;
        uint32_t buf_idx = gstate.cell_idx + idx;
        uint32_t coord = (buf_idx / data.tile_size[3]) % data.tile_size[2];
        vec[vec_idx] = offset + coord;
      }
    } else if (dest == 3) {
      auto vec = FlatVector::GetData<uint32_t>(output.data[i]);
      ...
      for (uint64_t idx = 0; idx < local_remains; idx++) {
        uint32_t vec_idx = current_filled + idx;
        uint32_t buf_idx = gstate.cell_idx + idx;
        uint32_t coord = buf_idx % data.tile_size[3];
        vec[vec_idx] = offset + coord;
      }
    } else if (dest == 4) {
      auto vec = FlatVector::GetData<double>(output.data[i]);
...
uint64_t DenseReader::_Put4DDataNoPrune(
    optional_ptr<const FunctionData> bind_data, ArrayReadGlobalState &gstate,
    char *pagevals, uint64_t size, DataChunk &output) {
  auto &data = bind_data->Cast<ArrayReadData>();
  uint64_t current_filled = output.size();
...
uint64_t DenseReader::_Put4DDataNoPruneAndProjection(
    optional_ptr<const FunctionData> bind_data, ArrayReadGlobalState &gstate,
    char *pagevals, uint64_t size, DataChunk &output) {
  auto &data = bind_data->Cast<ArrayReadData>();
  uint64_t current_filled = output.size();
  ...
  auto ys = FlatVector::GetData<uint32_t>(output.data[1]);
  auto zs = FlatVector::GetData<uint32_t>(output.data[2]);
  auto ws = FlatVector::GetData<uint32_t>(output.data[3]);
  auto vals =
      FlatVector::GetData<double>(output.data[4]);  // double type assumed
...
uint64_t DenseReader::_Put4DNullableData(
    optional_ptr<const FunctionData> bind_data, ArrayReadGlobalState &gstate,
    char *pagevals, uint64_t size, DataChunk &output) {
  auto &data = bind_data->Cast<ArrayReadData>();
  uint64_t max_processable = (uint64_t)STANDARD_VECTOR_SIZE;
...
uint64_t DenseReader::_Put4DNullableDataNoPrune(
    optional_ptr<const FunctionData> bind_data, ArrayReadGlobalState &gstate,
    char *pagevals, uint64_t size, DataChunk &output) {
  auto &data = bind_data->Cast<ArrayReadData>();
  uint64_t max_processable = (uint64_t)STANDARD_VECTOR_SIZE;
...
uint64_t DenseReader::_Put4DNullableDataNoPruneAndProjection(
    optional_ptr<const FunctionData> bind_data, ArrayReadGlobalState &gstate,
    char *pagevals, uint64_t size, DataChunk &output) {
  auto &data = bind_data->Cast<ArrayReadData>();
  uint64_t max_processable = (uint64_t)STANDARD_VECTOR_SIZE;
...
uint64_t DenseReader::PutData(optional_ptr<const FunctionData> bind_data,
                              ArrayReadGlobalState &gstate, char *pagevals,
                              uint64_t size, DataChunk &output) {
  auto &data = bind_data->Cast<ArrayReadData>();
  ...
  if (data.dim_len == 4) {
    if (nullable) {
      if (gstate.projection_ids.size() > 0)  // filter_prune ON
        return _Put4DNullableData(bind_data, gstate, pagevals, size, output);
      else if (gstate.column_ids.size() ==
               output.data.size())  // no filter prune
        return _Put4DNullableDataNoPrune(bind_data, gstate, pagevals, size,
                                         output);
      else  // projection_pushdown and filter_prune are both false
        return _Put4DNullableDataNoPruneAndProjection(bind_data, gstate,
                                                      pagevals, size, output);
    } else {
      if (gstate.projection_ids.size() > 0)  // filter_prune ON
        return _Put4DData(bind_data, gstate, pagevals, size, output);
      else if (gstate.column_ids.size() ==
               output.data.size())  // no filter prune
        return _Put4DDataNoPrune(bind_data, gstate, pagevals, size, output);
      else  // projection_pushdown and filter_prune are both false
        return _Put4DDataNoPruneAndProjection(bind_data, gstate, pagevals, size,
                                              output);
    }
  } else if (data.dim_len == 3) {
    if (nullable) {
      if (gstate.projection_ids.size() > 0)  // filter_prune ON
        return _Put3DNullableData(bind_data, gstate, pagevals, size, output);
      else if (gstate.column_ids.size() ==
               output.data.size())  // no filter prune
        return _Put3DNullableDataNoPrune(bind_data, gstate, pagevals, size,
                                         output);
      else  // projection_pushdown and filter_prune are both false
        return _Put3DNullableDataNoPruneAndProjection(bind_data, gstate,
                                                      pagevals, size, output);
    } else {
      if (gstate.projection_ids.size() > 0)  // filter_prune ON
        ...
        return _Put3DDataNoPruneAndProjection(bind_data, gstate, pagevals, size,
                                              output);
    }
    ...
    if (nullable) {
      if (gstate.projection_ids.size() > 0)  // filter_prune ON
        return _Put2DNullableData(bind_data, gstate, pagevals, size, output);
               ...
               output.data.size())  // no filter prune
        return _Put2DNullableDataNoPrune(bind_data, gstate, pagevals, size,
                                         output);
      else  // projection_pushdown and filter_prune are both false
        return _Put2DNullableDataNoPruneAndProjection(bind_data, gstate,
                                                      pagevals, size, output);
    } else {
      if (gstate.projection_ids.size() > 0)  // filter_prune ON
        ...
        return _Put2DDataNoPruneAndProjection(bind_data, gstate, pagevals, size,
                                              output);
    }
  } else {
    if (nullable) {
    ...
    if (gstate.projection_ids.size() > 0)  // filter_prune ON
      ...
      return _Put1DDataNoPruneAndProjection(bind_data, gstate, pagevals, size,
                                            output);
  }
}
}  // namespace duckdb