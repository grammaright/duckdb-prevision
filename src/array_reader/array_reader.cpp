...
uint64_t ArrayReader::_Put4DData(optional_ptr<const FunctionData> bind_data,
                                 ArrayReadGlobalState &gstate, char *pagevals,
                                 uint64_t size, DataChunk &output) {
    auto &data = bind_data->Cast<ArrayReadData>();
    uint64_t current_filled = output.size();
    auto total_remains = size - gstate.cell_idx;
    auto local_remains = std::min((uint64_t)STANDARD_VECTOR_SIZE - current_filled, total_remains);
    ...
    for (uint32_t i = 0; i < gstate.projection_ids.size(); i++) {
        ...
        if (dest == 0) {
            ...
            for (uint64_t idx = 0; idx < local_remains; idx++) {
                uint32_t vec_idx = current_filled + idx;
                uint32_t buf_idx = gstate.cell_idx + idx;
                uint32_t coord = buf_idx / (data.tile_size[1] * data.tile_size[2] * data.tile_size[3]);
                vec[vec_idx] = offset + coord;
            }
        } else if (dest == 1) {
            auto vec = FlatVector::GetData<uint32_t>(output.data[i]);
            ...
            for (uint64_t idx = 0; idx < local_remains; idx++) {
                uint32_t vec_idx = current_filled + idx;
                uint32_t buf_idx = gstate.cell_idx + idx;
                uint32_t coord = (buf_idx / (data.tile_size[2] * data.tile_size[3])) % data.tile_size[1];
                vec[vec_idx] = offset + coord;
            }
        } else if (dest == 2) {
            auto vec = FlatVector::GetData<uint32_t>(output.data[i]);
            ...
            auto vec = FlatVector::GetData<double>(output.data[i]);
            memcpy(vec + current_filled, ((double *)pagevals) + gstate.cell_idx, sizeof(double) * local_remains);
        }
    }
...
uint64_t ArrayReader::_Put4DDataNoPrune(
    optional_ptr<const FunctionData> bind_data, ArrayReadGlobalState &gstate,
    char *pagevals, uint64_t size, DataChunk &output) {
    auto &data = bind_data->Cast<ArrayReadData>();
    uint64_t current_filled = output.size();
    ...
    for (uint64_t idx = 0; idx < local_remains; idx++) {
        uint64_t *coords;
        uint64_t buf_idx = gstate.cell_idx + idx;
        bf_util_calculate_nd_from_1d_row_major(buf_idx, (uint64_t *)data.tile_size.data(), data.dim_len, &coords);
        double val = ((double *)pagevals)[buf_idx];

        auto vec_idx = current_filled + idx;
        for (uint32_t i = 0; i < gstate.column_ids.size(); i++) {
            if (gstate.column_ids[i] == 0)
                FlatVector::GetData<uint32_t>(output.data[i])[vec_idx] = offsets[0] + coords[0];
            else if (gstate.column_ids[i] == 1)
                FlatVector::GetData<uint32_t>(output.data[i])[vec_idx] = offsets[1] + coords[1];
            else if (gstate.column_ids[i] == 2)
                FlatVector::GetData<uint32_t>(output.data[i])[vec_idx] = offsets[2] + coords[2];
            else if (gstate.column_ids[i] == 3)
                FlatVector::GetData<uint32_t>(output.data[i])[vec_idx] = offsets[3] + coords[3];
...
uint64_t ArrayReader::_Put4DDataNoPruneAndProjection(
    optional_ptr<const FunctionData> bind_data, ArrayReadGlobalState &gstate,
    char *pagevals, uint64_t size, DataChunk &output) {
    auto &data = bind_data->Cast<ArrayReadData>();
    uint64_t current_filled = output.size();
    ...
    auto vals = FlatVector::GetData<double>(output.data[4]);  // double type assumed
...
uint64_t ArrayReader::_Put4DNullableData(optional_ptr<const FunctionData> bind_data,
                                         ArrayReadGlobalState &gstate, char *pagevals,
                                         uint64_t size, DataChunk &output) {
    auto &data = bind_data->Cast<ArrayReadData>();
    uint64_t max_processable = (uint64_t)STANDARD_VECTOR_SIZE;
...
uint64_t ArrayReader::_Put4DNullableDataNoPrune(
    optional_ptr<const FunctionData> bind_data, ArrayReadGlobalState &gstate,
    char *pagevals, uint64_t size, DataChunk &output) {
    auto &data = bind_data->Cast<ArrayReadData>();
    uint64_t max_processable = (uint64_t)STANDARD_VECTOR_SIZE;
    ...
    while (idx < max_processable && buf_idx < size) {
        ...
        for (uint32_t i = 0; i < gstate.column_ids.size(); i++) {
            if (gstate.column_ids[i] == 0)
                FlatVector::GetData<uint32_t>(output.data[i])[idx] = offsets[0] + coords[0];
            else if (gstate.column_ids[i] == 1)
                FlatVector::GetData<uint32_t>(output.data[i])[idx] = offsets[1] + coords[1];
            else if (gstate.column_ids[i] == 2)
                FlatVector::GetData<uint32_t>(output.data[i])[idx] = offsets[2] + coords[2];
            else if (gstate.column_ids[i] == 3)
                FlatVector::GetData<uint32_t>(output.data[i])[idx] = offsets[3] + coords[3];
            else if (gstate.column_ids[i] == 4)
                FlatVector::GetData<double>(output.data[i])[idx] = val;
        }
        buf_idx++;
...
uint64_t ArrayReader::_Put4DNullableDataNoPruneAndProjection(
    optional_ptr<const FunctionData> bind_data, ArrayReadGlobalState &gstate,
    char *pagevals, uint64_t size, DataChunk &output) {
    auto &data = bind_data->Cast<ArrayReadData>();
    uint64_t max_processable = (uint64_t)STANDARD_VECTOR_SIZE;
    ...
    if (data.dim_len == 4) {
        if (nullable) {
            if (gstate.projection_ids.size() > 0)  // filter_prune ON
                ...
                return _Put4DNullableDataNoPrune(bind_data, gstate, pagevals, size, output);
            else  // projection_pushdown and filter_prune are both false
                return _Put4DNullableDataNoPruneAndProjection(bind_data, gstate, pagevals,
                                                    size, output);
        } else {
            if (gstate.projection_ids.size() > 0)  // filter_prune ON
                ...
                return _Put4DDataNoPruneAndProjection(bind_data, gstate, pagevals,
                                                    size, output);
        }
